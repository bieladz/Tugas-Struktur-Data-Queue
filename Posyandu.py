...
def speak(text: str):
    """Pakai Google TTS untuk suara Indonesia yang natural"""
    def _speak():
        try:
            ...
            # Buat file temporary untuk audio
            with tempfile.NamedTemporaryFile(delete=False, suffix='.mp3') as f:
                temp_file = f.name
            
            # Generate suara dengan bahasa Indonesia
            ...
            print("[TTS] Install library: pip install gtts playsound")
        except Exception as e:
            print(f"[TTS Error] {e}")
            # Fallback ke pyttsx3 jika gagal
            try:
                import pyttsx3
                engine = pyttsx3.init()
                engine.say(text)
...
class PosyanduApp:
    ...
    def _build_ui(self):
        ...
        self._btn(btn_row, "🗒️  Tampilkan Antrian", c['accent'], self._tampilkan_antrian, side='left')
        ...
        self.statusbar = tk.Label(self.root, text="Selamat Datang di Posyandu.",
                          font=('Segoe UI', 10), bg=c['header_bg'],  # Gunakan c['header_bg'] bukan "#008080"
                          fg=c['header_fg'], anchor='w', padx=10, pady=4)
        ...
        style.map('Treeview', background=[('selected', '#4988C4')])
